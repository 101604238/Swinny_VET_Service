...
GO
PRINT N'Dropping [dbo].[FK_Treatment_PetName]...';


GO
ALTER TABLE [dbo].[Treatment] DROP CONSTRAINT [FK_Treatment_PetName];


GO
PRINT N'Dropping [dbo].[FK_Treatment_ProcedureID]...';


GO
ALTER TABLE [dbo].[Treatment] DROP CONSTRAINT [FK_Treatment_ProcedureID];
...
CREATE TABLE [dbo].[tmp_ms_xx_Treatment] (
    ...
    CONSTRAINT [tmp_ms_xx_constraint_PK_Treatment1] PRIMARY KEY CLUSTERED ([PetName] ASC, [OwnerID] ASC, [ProcedureID] ASC, [TreatmentDate] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[Treatment])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_Treatment] ([PetName], [OwnerID], [ProcedureID], [TreatmentDate], [TreatmentNotes], [TreatmentPrice])
        SELECT   [PetName],
                 [OwnerID],
                 [ProcedureID],
                 [TreatmentDate],
                 ...
                 [TreatmentPrice]
        FROM     [dbo].[Treatment]
        ORDER BY [PetName] ASC, [OwnerID] ASC, [ProcedureID] ASC, [TreatmentDate] ASC;
    END
...
EXECUTE sp_rename N'[dbo].[tmp_ms_xx_constraint_PK_Treatment1]', N'PK_Treatment', N'OBJECT';
